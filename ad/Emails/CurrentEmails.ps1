...
.DESCRIPTION
    ...
    distribution groups.  Used for verifying there are no unnecessary email
    accounts in systems that aren't synchronized with Active Directory
...
foreach($file in $files)
{
    ...
    $noExclist = Compare-Object -ReferenceObject $list -DiferenceObject $ignore |
    ...
    foreach($user in $noExclist)
    {
        ...
        if($null -ne $check)
        {
            ...
            $disabled = $( try{ Get-ADUser -Filter "$SAM -and Enabled -eq '$TRUE'" } catch{ $null } )
            #If $disabled is not null, then output the user to file
            if($null -ne $disabled)
            {
                if($file -eq $files[0]) { $user = $user + "<emaildomain1>" }
                elseif($file -eq $files[1]) { $user = $user + "<emaildomain2>" }
                Out-file -FilePath "$OutFile" -InputObject "$user" -Encoding ASCII -Append
            }
        }
    }
}
